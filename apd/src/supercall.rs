...
const SUPERCALL_SU_CAP_SET_PROFILE: c_long = 0x1107;
const SUPERCALL_SU_CAP_DEL_PROFILE: c_long = 0x1109;
...
fn sc_su_cap_set_profile(key: &CStr, profile: &CapProfile) -> c_long {
    if key.to_bytes().is_empty() {
        return (-EINVAL).into();
    }
    unsafe {
        syscall(
            __NR_SUPERCALL,
            key.as_ptr(),
            ver_and_cmd(SUPERCALL_SU_CAP_SET_PROFILE),
            profile,
        ) as c_long
    }
}

fn sc_su_cap_del_profile(key: &CStr, uid: uid_t) -> c_long {
    if key.to_bytes().is_empty() {
        return (-EINVAL).into();
    }
    unsafe {
        syscall(
            __NR_SUPERCALL,
            key.as_ptr(),
            ver_and_cmd(SUPERCALL_SU_CAP_DEL_PROFILE),
            uid,
        ) as c_long
    }
}
...
pub fn refresh_ap_package_list(skey: &CStr, mutex: &Arc<Mutex<()>>) {
    ...
    for config in package_configs {
        ...
        if config.cap_enabled != 0 {
            let cap_profile = CapProfile {
                uid: config.uid,
                cap_enable: config.cap_enabled,
                cap_inheritable: config.cap_inheritable,
                cap_permitted: config.cap_permitted,
                cap_effective: config.cap_effective,
                cap_bset: config.cap_bset,
                cap_ambient: config.cap_ambient,
            };
            let result = sc_su_cap_set_profile(skey, &cap_profile);
            info!(
                "[refresh_ap_package_list] Loading cap {}: result = {}",
                config.pkg, result
            );
        } else {
            let result = sc_su_cap_del_profile(skey, config.uid as uid_t);
            info!(
                "[refresh_ap_package_list] Clearing cap {}: result = {}",
                config.pkg, result
            );
        }
    }
        ...
        if config.cap_enabled != 0 {
            match key {
                Some(ref key) => {
                    ...
                    let result = sc_su_cap_set_profile(key, &cap_profile);
                    info!("Processed cap {}: result = {}", config.pkg, result);
                }
                _ => {
                    warn!("Superkey is None, skipping cap config: {}", config.pkg);
                }
            }
        } else {
            match key {
                Some(ref key) => {
                    let result = sc_su_cap_del_profile(key, config.uid as uid_t);
                    info!("Processed cap clear {}: result = {}", config.pkg, result);
                }
                _ => {
                    warn!("Superkey is None, skipping cap clear config: {}", config.pkg);
                }
            }
        }
    }
