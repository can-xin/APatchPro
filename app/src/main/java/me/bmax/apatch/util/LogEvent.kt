...
fun getBugreportFile(context: Context): File {
    ...
    shell.newJob().add("echo \"uid: \\$(id)\" > ${dmesgFile.absolutePath}").exec()
    shell.newJob().add("echo \"whoami: \\$(whoami)\" >> ${dmesgFile.absolutePath}").exec()
    shell.newJob().add("(dmesg 2>/dev/null || /data/adb/ap/bin/busybox dmesg 2>/dev/null || cat /proc/kmsg 2>/dev/null || cat /proc/last_kmsg 2>/dev/null || cat /sys/fs/pstore/console-ramoops* 2>/dev/null || echo \"[APatch] kernel log unavailable\") >> ${dmesgFile.absolutePath}").exec()
    shell.newJob().add("logcat -b kernel -d >> ${dmesgFile.absolutePath} 2>/dev/null || true").exec()
