...
object Version {
    ...
    private fun installedApdVString(): String {
        val resultShell = rootShellForResult("${APApplication.APD_PATH} -V 2>/dev/null")
        if (resultShell.isSuccess) {
            ...
            val parsed = Regex("\\d+").find(result)?.value
            if (!parsed.isNullOrEmpty()) {
                installedApdVString = parsed
                return installedApdVString
            }
        }

        val fallbackShell = rootShellForResult("cat ${APApplication.APATCH_FOLDER}version 2>/dev/null")
        val fallback = fallbackShell.out.joinToString("\n").trim()
        installedApdVString = Regex("\\d+").find(fallback)?.value ?: "0"
        return installedApdVString
    }
