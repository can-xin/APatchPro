...
@JvmOverloads
fun createRootShell(globalMnt: Boolean = false, authKey: String = APApplication.superKey): Shell {
    Shell.enableVerboseLogging = BuildConfig.DEBUG
    val builder = Shell.Builder.create().setInitializers(RootShellInitializer::class.java)
    return try {
        builder.build(
            SUPERCMD, authKey, "-Z", APApplication.MAGISK_SCONTEXT
        )
    } catch (e: Throwable) {
        ...
        return try {
            ...
            if (globalMnt) {
                builder.build(
                    getKPatchPath(), authKey, "su", "-Z", APApplication.MAGISK_SCONTEXT, "--mount-master"
                )
            }else{
                builder.build(
                    getKPatchPath(), authKey, "su", "-Z", APApplication.MAGISK_SCONTEXT
                )
            }
        } catch (e: Throwable) {
...
inline fun <T> withNewRootShell(
    ...
    authKey: String = APApplication.superKey,
    block: Shell.() -> T
): T {
    return createRootShell(globalMnt, authKey).use(block)
}
