...
class APApplication : Application(), Thread.UncaughtExceptionHandler {
    ...
    companion object {
        ...
        fun uninstallApatch(authKey: String = superKey) {
            ...
            val result = withNewRootShell(authKey = superKey) {
                newJob().add(*cmds).to(logCallback, logCallback).exec()
            }

            APatchCli.refresh()
            val installedAfterUninstall = Version.installedApdVUInt() > 0
            if (!installedAfterUninstall) {
                ...
                Log.e(TAG, "APatch uninstall failed, code=${result.code}, detectedInstalled=$installedAfterUninstall")
                _apStateLiveData.postValue(State.ANDROIDPATCH_INSTALLED)
            }
        }

        @Suppress("DEPRECATION")
        fun installApatch(authKey: String = superKey) {
            ...
            val apdProbeResult = rootShellForResult("$APD_PATH -V 2>/dev/null")
            val apdResponsive = apdProbeResult.isSuccess && apdProbeResult.out.joinToString("\n").trim().isNotEmpty()
            val installedAfterInstall = apdResponsive
            if (installedAfterInstall) {
                Log.d(TAG, "APatch installed...")
                _apStateLiveData.postValue(State.ANDROIDPATCH_INSTALLED)
            } else {
                Log.e(TAG, "APatch install failed, code=${result.code}, apdResponsive=$apdResponsive")
                _apStateLiveData.postValue(State.ANDROIDPATCH_NOT_INSTALLED)
            }
        }

        fun markNeedReboot(authKey: String) {
            val result = withNewRootShell(authKey = superKey) {
                newJob().add("touch $NEED_REBOOT_FILE").exec()
            }
        ...
        var superKey: String = ""
            set(value) {
                ...
                thread {
                    ...
                    if (!rc) {
                        Log.w(TAG, "Native.su probe failed, continue APD detection")
                    }
                    ...
                    val apdProbeResult = rootShellForResult("$APD_PATH -V 2>/dev/null")
                    val apdResponsive = apdProbeResult.isSuccess && apdProbeResult.out.joinToString("\n").trim().isNotEmpty()
                    Log.d(TAG, "manager version: $mgv, apd responsive: $apdResponsive")
        ...
        var keyAuthMode: KeyAuthMode = KeyAuthMode.DUAL
            set(value) {
                field = KeyAuthMode.DUAL
                APatchKeyHelper.writeKeyAuthMode(1)
            }

        fun isDualKeyMode(): Boolean {
            return true
        }

        fun composeWriteKey(key2: String): String {
            return superKey + key2
        }
    }
    ...
    override fun onCreate() {
        ...
        val enforceOfficialSignature = packageName == "me.bmax.apatch"
        if (!BuildConfig.DEBUG && enforceOfficialSignature && !verifyAppSignature("1x2twMoHvfWUODv7KkRRNKBzOfEqJwRKGzJpgaz18xk=")) {
            while (true) {
