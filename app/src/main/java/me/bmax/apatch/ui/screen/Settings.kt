...
import me.bmax.apatch.ui.theme.GlobalBackgroundConfig
...
fun SettingScreen() {
    ...
    Scaffold(
        ...
        var showPanicKey2Dialog by remember { mutableStateOf(false) }
        var panicKey2 by remember { mutableStateOf("") }
        ...
        var globalBackgroundEnabled by rememberSaveable {
            mutableStateOf(GlobalBackgroundConfig.isEnabled(context))
        }
        var hasGlobalBackground by rememberSaveable {
            mutableStateOf(!GlobalBackgroundConfig.getBackgroundUri(context).isNullOrBlank())
        }
        ...
        val pickGlobalBackgroundLauncher = rememberLauncherForActivityResult(
            ActivityResultContracts.GetContent()
        ) { uri: Uri? ->
            if (uri != null) {
                scope.launch(Dispatchers.IO) {
                    val ok = GlobalBackgroundConfig.saveBackground(context, uri)
                    withContext(Dispatchers.Main) {
                        if (ok) {
                            globalBackgroundEnabled = true
                            hasGlobalBackground = true
                            refreshTheme.value = true
                            snackBarHost.showSnackbar(context.getString(R.string.settings_global_background_set_success))
                        } else {
                            snackBarHost.showSnackbar(context.getString(R.string.settings_global_background_set_failed))
                        }
                    }
                }
            }
        ...
        Column(
                    ...
                    if (it) {
                        ...
                        if (canAuthenticate == BiometricManager.BIOMETRIC_SUCCESS) {
                            APApplication.sharedPreferences.edit {
                                putBoolean("biometric_login", true)
                            }
                            biometricLogin = true
                        } else {
                            Toast.makeText(
            ...
            SwitchItem(
                ...
                title = stringResource(id = R.string.settings_global_background_enable),
                summary = stringResource(id = R.string.settings_global_background_enable_summary),
                checked = globalBackgroundEnabled,
                onCheckedChange = {
                    globalBackgroundEnabled = it
                    GlobalBackgroundConfig.setEnabled(context, it)
                    refreshTheme.value = true
                }
            )
            ...
            ListItem(
                headlineContent = {
                    Text(text = stringResource(id = R.string.settings_global_background_pick))
                },
                supportingContent = {
                    Text(
                        text = if (hasGlobalBackground) {
                            stringResource(id = R.string.settings_global_background_selected)
                        } else {
                            stringResource(id = R.string.settings_global_background_not_selected)
                        },
                        style = MaterialTheme.typography.bodyMedium,
                        color = MaterialTheme.colorScheme.outline
                    )
                ...
                modifier = Modifier.clickable {
                    pickGlobalBackgroundLauncher.launch("image/*")
                },
                leadingContent = { Icon(Icons.Filled.FormatColorFill, null) }
            )
            ...
            ListItem(
                headlineContent = {
                    Text(text = stringResource(id = R.string.settings_global_background_clear))
                },
                modifier = Modifier.clickable {
                    GlobalBackgroundConfig.clear(context)
                ...
                ListItem(
                    leadingContent = {
                        Icon(
                            ...
                            stringResource(id = R.string.setting_kernel_panic)
                        )
                    },
                    supportingContent = {
                        Text(
                            text = stringResource(id = R.string.setting_kernel_panic_summary),
                            style = MaterialTheme.typography.bodyMedium,
                            color = MaterialTheme.colorScheme.outline
                        )
                    },
                    headlineContent = { Text(stringResource(id = R.string.setting_kernel_panic)) },
                    modifier = Modifier.clickable {
            ...
            BasicAlertDialog(
                ...
                Surface(
                    ...
                    Column(modifier = Modifier.padding(PaddingValues(all = 24.dp))) {
                        Text(
                            text = stringResource(id = R.string.setting_kernel_panic_confirm_title),
                            style = MaterialTheme.typography.headlineSmall
                        )
                        Text(
                            modifier = Modifier.padding(top = 12.dp),
                            text = stringResource(id = R.string.setting_kernel_panic_confirm_content),
                            style = MaterialTheme.typography.bodyMedium
                        )
                        Row(
                            modifier = Modifier
        ...
        if (showPanicKey2Dialog) {
            BasicAlertDialog(
                onDismissRequest = { showPanicKey2Dialog = false },
                properties = DialogProperties(
                    decorFitsSystemWindows = true,
                    usePlatformDefaultWidth = false,
                )
                ...
                Surface(
                    ...
                    Column(modifier = Modifier.padding(PaddingValues(all = 24.dp))) {
                        Text(text = stringResource(id = R.string.key2_dialog_title))
                        OutlinedTextField(
                            modifier = Modifier
                                .fillMaxWidth()
                                .padding(top = 12.dp),
                            value = panicKey2,
                            onValueChange = { panicKey2 = it },
                            label = { Text(stringResource(id = R.string.patch_set_key2)) },
                        ...
                        Row(
                            ...
                            TextButton(onClick = { showPanicKey2Dialog = false }) {
                                Text(stringResource(id = android.R.string.cancel))
                            }
                            Button(
                                onClick = {
                                    showPanicKey2Dialog = false
                                    val rc = Natives.panic(APApplication.composeWriteKey(panicKey2))
                                    Toast.makeText(
                                        context,
                                        if (rc == 0L) R.string.success else R.string.setting_kernel_panic_failed,
                                        Toast.LENGTH_SHORT
                                    ).show()
                                },
                                enabled = panicKey2.length in 3..10 && panicKey2.all { it.isDigit() }
                            ) {
                                Text(stringResource(id = android.R.string.ok))
                            }
                        }
...
fun ResetSUPathDialog(showDialog: MutableState<Boolean>) {
    ...
    fun applySuPath(authKey: String): Boolean {
        val success = Natives.resetSuPath(authKey, suPath)
        Toast.makeText(
            context,
    ...
    if (showWriteKeyDialog) {
        BasicAlertDialog(
            onDismissRequest = { showWriteKeyDialog = false },
            properties = DialogProperties(
                decorFitsSystemWindows = true,
                usePlatformDefaultWidth = false,
            )
            ...
            Surface(
                ...
                Column(modifier = Modifier.padding(PaddingValues(all = 24.dp))) {
                    ...
                    Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.End) {
                        TextButton(onClick = { showWriteKeyDialog = false }) {
                            Text(stringResource(id = android.R.string.cancel))
                        }
                        Button(
                            onClick = {
                                showWriteKeyDialog = false
                                applySuPath(APApplication.composeWriteKey(writeKey2))
                                showDialog.value = false
                            },
