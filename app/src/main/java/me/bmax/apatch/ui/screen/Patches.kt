...
private fun SetSuperKeyView(viewModel: PatchesViewModel) {
    ...
    var key2 by remember { mutableStateOf(viewModel.key2) }
    var key2Visible by remember { mutableStateOf(false) }
    var showKey2Warn by remember { mutableStateOf(!viewModel.checkKey2Validation(key2)) }
        ...
        Column(
            ...
            Column {
                ...
                if (showKey2Warn) {
                    Text(
                        color = Color.Red,
                        text = stringResource(id = R.string.patch_item_set_key2_label),
                        style = MaterialTheme.typography.bodyMedium
                    )
                }
                Box(contentAlignment = Alignment.CenterEnd) {
                    OutlinedTextField(
                        ...
                        onValueChange = {
                            key2 = it
                            viewModel.key2 = it
                            showKey2Warn = !viewModel.checkKey2Validation(it)
                        },
                        label = { Text(stringResource(id = R.string.patch_set_key2)) },
                        visualTransformation = if (key2Visible) VisualTransformation.None else PasswordVisualTransformation(),
                    ...
                    IconButton(
                        modifier = Modifier
                            .size(40.dp)
                            .padding(top = 15.dp, end = 5.dp),
                        onClick = { key2Visible = !key2Visible }
                    ) {
                        Icon(
                            imageVector = if (key2Visible) Icons.Default.Visibility else Icons.Default.VisibilityOff,
                            contentDescription = null,
                            tint = Color.Gray
                        )
                    }
