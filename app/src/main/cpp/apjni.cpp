...
jobject nativeSuCapProfile(JNIEnv *env, jobject /* this */, jstring super_key_jstr, jint uid) {
    ensureSuperKeyNonNull(super_key_jstr);

    const auto super_key = JUTFString(env, super_key_jstr);
    struct cap_profile profile{};
    long rc = sc_su_cap_get_profile(super_key.get(), (uid_t) uid, &profile);
    if (rc < 0) [[unlikely]] {
        return nullptr;
    }

    jclass cls = env->FindClass("me/bmax/apatch/Natives$CapProfile");
    if (!cls) [[unlikely]] {
        LOGE("nativeSuCapProfile: CapProfile class not found");
        return nullptr;
    }

    jmethodID constructor = env->GetMethodID(cls, "<init>", "()V");
    jfieldID uidField = env->GetFieldID(cls, "uid", "I");
    jfieldID enabledField = env->GetFieldID(cls, "enabled", "I");
    jfieldID inheritableField = env->GetFieldID(cls, "inheritable", "J");
    jfieldID permittedField = env->GetFieldID(cls, "permitted", "J");
    jfieldID effectiveField = env->GetFieldID(cls, "effective", "J");
    jfieldID bsetField = env->GetFieldID(cls, "bset", "J");
    jfieldID ambientField = env->GetFieldID(cls, "ambient", "J");

    jobject obj = env->NewObject(cls, constructor);
    env->SetIntField(obj, uidField, (jint) profile.uid);
    env->SetIntField(obj, enabledField, (jint) profile.cap_enable);
    env->SetLongField(obj, inheritableField, (jlong) profile.cap_inheritable);
    env->SetLongField(obj, permittedField, (jlong) profile.cap_permitted);
    env->SetLongField(obj, effectiveField, (jlong) profile.cap_effective);
    env->SetLongField(obj, bsetField, (jlong) profile.cap_bset);
    env->SetLongField(obj, ambientField, (jlong) profile.cap_ambient);

    return obj;
}

jlong nativeSetSuCapProfile(JNIEnv *env, jobject /* this */, jstring super_key_jstr, jobject cap_profile_obj) {
    ensureSuperKeyNonNull(super_key_jstr);

    if (!cap_profile_obj) [[unlikely]] {
        return -EINVAL;
    }

    const auto super_key = JUTFString(env, super_key_jstr);
    jclass cls = env->GetObjectClass(cap_profile_obj);
    if (!cls) [[unlikely]] {
        return -EINVAL;
    }
    ...
    struct cap_profile profile{};
    profile.uid = (uid_t) env->GetIntField(cap_profile_obj, uidField);
    profile.cap_enable = (int) env->GetIntField(cap_profile_obj, enabledField);
    profile.cap_inheritable = (uint64_t) env->GetLongField(cap_profile_obj, inheritableField);
    profile.cap_permitted = (uint64_t) env->GetLongField(cap_profile_obj, permittedField);
    profile.cap_effective = (uint64_t) env->GetLongField(cap_profile_obj, effectiveField);
    profile.cap_bset = (uint64_t) env->GetLongField(cap_profile_obj, bsetField);
    profile.cap_ambient = (uint64_t) env->GetLongField(cap_profile_obj, ambientField);

    return sc_su_cap_set_profile(super_key.get(), &profile);
}

jlong nativeDeleteSuCapProfile(JNIEnv *env, jobject /* this */, jstring super_key_jstr, jint uid) {
    ensureSuperKeyNonNull(super_key_jstr);

    const auto super_key = JUTFString(env, super_key_jstr);
    return sc_su_cap_del_profile(super_key.get(), (uid_t) uid);
}
...
jlong nativePanic(JNIEnv *env, jobject /* this */, jstring super_key_jstr) {
    ensureSuperKeyNonNull(super_key_jstr);

    const auto super_key = JUTFString(env, super_key_jstr);
    return sc_panic(super_key.get());
}
...
JNIEXPORT jint JNI_OnLoad(JavaVM* vm, void * /*reserved*/) {
    ...
    const static JNINativeMethod gMethods[] = {
        ...
        {"nativeSuCapProfile", "(Ljava/lang/String;I)Lme/bmax/apatch/Natives$CapProfile;", reinterpret_cast<void *>(&nativeSuCapProfile)},
        {"nativeSetSuCapProfile", "(Ljava/lang/String;Lme/bmax/apatch/Natives$CapProfile;)J", reinterpret_cast<void *>(&nativeSetSuCapProfile)},
        {"nativeDeleteSuCapProfile", "(Ljava/lang/String;I)J", reinterpret_cast<void *>(&nativeDeleteSuCapProfile)},
        ...
        {"nativePanic", "(Ljava/lang/String;)J", reinterpret_cast<void *>(&nativePanic)},
    };
