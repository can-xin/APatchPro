...
static inline long sc_su_cap_set_profile(const char *key, struct cap_profile *profile)
{
    if (!key || !key[0]) return -EINVAL;
    if (!profile) return -EINVAL;
    long ret = syscall(__NR_supercall, key, ver_and_cmd(key, SUPERCALL_SU_CAP_SET_PROFILE), profile);
    return ret;
}

static inline long sc_su_cap_get_profile(const char *key, uid_t uid, struct cap_profile *out_profile)
{
    if (!key || !key[0]) return -EINVAL;
    if (!out_profile) return -EINVAL;
    long ret = syscall(__NR_supercall, key, ver_and_cmd(key, SUPERCALL_SU_CAP_GET_PROFILE), uid, out_profile);
    return ret;
}

static inline long sc_su_cap_del_profile(const char *key, uid_t uid)
{
    if (!key || !key[0]) return -EINVAL;
    long ret = syscall(__NR_supercall, key, ver_and_cmd(key, SUPERCALL_SU_CAP_DEL_PROFILE), uid);
    return ret;
}
